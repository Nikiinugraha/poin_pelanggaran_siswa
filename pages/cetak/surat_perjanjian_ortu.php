...
if(isset($_GET['nis'])){
    ...
    while($id_pelanggaran_siswa = mysqli_fetch_assoc($pelanggaran_siswa)){
        ...
        if(mysqli_num_rows($cek_data) == 0){
            echo "<script>alert('Siswa melakukan pelanggaran baru, silahkan cetak surat lagi, dengan memilih nis " . $nis . "'); window.location.href = '/poin_pelanggaran_siswa/pages/cetak/add_perjanjian_ortu.php';</script>";
            exit;
        }
    }
    ...
    if(mysqli_num_rows(mysqli_query($conn, "SELECT * FROM pelanggaran_siswa WHERE nis = '$nis'")) == 0){
        echo "<script>alert('Data siswa belum memiliki data pelanggaran 👍'); window.location.href = '/poin_pelanggaran_siswa/pages/cetak/add_perjanjian_ortu.php';</script>";
        exit;
    }
...
// Jika diakses lewat Cetak Kembali (GET), maka sistem akan menebak peran orang tua 
// dengan mencocokkan nama_ortu hasil query dengan data ayah/ibu/wali di ortu_wali
if (!isset($orang_tua)) {
    if ($row_ortu_wali['nama_ortu'] == $row_ortu_wali['ayah']) {
        $orang_tua = "ayah";
    } else if ($row_ortu_wali['nama_ortu'] == $row_ortu_wali['ibu']) {
        $orang_tua = "ibu";
    } else {
        $orang_tua = "wali";
    }
...
<center class="no-print">
    ...
    <link rel="stylesheet" href="/poin_pelanggaran_siswa/css/pages/cetak/surat_perjanjian_ortu.css">
