...
<link rel="stylesheet" href="/poin_pelanggaran_siswa/css/pages/cetak/add_perjanjian_siswa.css">

<div class="add-perjanjian-wrapper">
    <header class="page-header-premium">
        <div class="header-top">
            <h2 class="animate-title">Surat Perjanjian Siswa</h2>
    ...
    <div class="parent-forms-grid animate-slide-up">
        ...
        function renderParentCard($type, $name, $job, $address, $phone, $nis, $id_ortu_wali) {
            $badge_class = "badge-" . strtolower($type);
            $icon = ($type == 'Ayah') ? 'fa-user-tie' : (($type == 'Ibu') ? 'fa-female' : 'fa-hand-holding-heart');
            ?>
            <div class="parent-card">
                <div class="header-card">
                    <span class="parent-type-badge <?= $badge_class ?>"><?= $type ?></span>
        ...
        if(!empty($row_ortu_wali["ayah"])) {
            renderParentCard('Ayah', $row_ortu_wali['ayah'], $row_ortu_wali['pekerjaan_ayah'], $row_ortu_wali['alamat_ayah'], $row_ortu_wali['no_telp_ayah'], $nis_query, $row_ortu_wali['id_ortu_wali']);
        }
        if(!empty($row_ortu_wali["ibu"])) {
            renderParentCard('Ibu', $row_ortu_wali['ibu'], $row_ortu_wali['pekerjaan_ibu'], $row_ortu_wali['alamat_ibu'], $row_ortu_wali['no_telp_ibu'], $nis_query, $row_ortu_wali['id_ortu_wali']);
        }
        if(!empty($row_ortu_wali["wali"])) {
            renderParentCard('Wali', $row_ortu_wali['wali'], $row_ortu_wali['pekerjaan_wali'], $row_ortu_wali['alamat_wali'], $row_ortu_wali['no_telp_wali'], $nis_query, $row_ortu_wali['id_ortu_wali']);
        }
        ...
        if(empty($row_ortu_wali["ayah"]) && empty($row_ortu_wali["ibu"]) && empty($row_ortu_wali["wali"])) {
            ...
            <div class="parent-card" style="grid-column: 1 / -1;">
                ...
                <form action="surat_perjanjian_siswa.php" method="post">
                    ...
                    <div class="form-grid-premium" style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                        <div class="form-group-premium">
                    ...
                    <button type="submit" class="btn-print-premium" style="max-width: 300px; margin: 20px auto 0;">
                        <i class="fas fa-print"></i> CETAK SURAT PERJANJIAN
                    </button>
                </form>
            </div>
